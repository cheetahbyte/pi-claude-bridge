...
- **Tests: `diag/ab-rebuild.mjs`** — drives one rebuild boundary over RPC so the prompt-cache cost of a rebuild can be read off `diag/capture-proxy.mjs` output. Measured at a ~67k-token prefix: upstream read back 8,422 and rewrote 58,779 tokens (12.5% hit); with the carry, 67,577 read and 171 written (99.7%).
- **Add: `provider.disableHooks`** — run Claude Code with `disableAllHooks`. Off by default. Useful under pi, which is the harness here, and it also protects the one rebuild that has no prior CC session to carry hook output from (`pi --continue` into a bridged session).
